...
export default function InputPastes(): JSX.Element {
  ...
  const [pasteObject, setPasteObject] = useState<pasteI>({
    name: "",
    pasteTitle: "",
    pasteContent: "",
  });
  ...
  const handleSubmit = async () => {
    ...
    const response = await axios.post("http://localhost:4000/pastes", {
      ...
      pasteTitle: pasteObject.pasteTitle,
      pasteContent: pasteObject.pasteContent,
    });
    ...
    <div>
      ...
      <div>
        ...
        <p>
          {" "}
          {pasteObject.name}
          {pasteObject.pasteContent}
          {pasteObject.pasteTitle}
        </p>
      </div>
    </div>
  );
